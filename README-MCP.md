...
- `hopx_spawn_agent` — one-call subagent bring-up: create terminal + launch agent CLI (claude/codex/gemini/custom) + wait until ready + optionally run a first task with an automatic completion contract; `isolation="worktree"` gives the worker its own git worktree + fleet/ branch for overlapping-file fleets
  - the Claude preset uses the `opus` model alias and `bypassPermissions` mode by default, and removes inherited Claude Code process credentials before launch, so a delegated worker uses the host's durable `claude auth login` without parking on approval prompts; pass `permission_mode="manual"` for a restricted worker, `args="--model ..."` to choose another model, or an explicit `command` when inherited credentials are intentional
  - the host operator can cap the mode with `hop config agent-ceiling <mode>` (or `HOP_AGENT_PERMISSION_CEILING`); a request above the cap is clamped, the applied mode is returned as `permission_mode`, and the clamp is explained in `permission_note`
  - the Codex preset runs autonomously and bypasses workspace-trust, approval, and self-update startup gates when the ceiling is `bypassPermissions` (the default); under any lower ceiling it runs `--full-auto` (sandboxed, asks on request). Hop rejects detected login, trust, or update screens instead of dispatching a task into them
