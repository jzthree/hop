...
#### Tooling
- **Deterministic turn-completion signal for agent driving**: the Claude hook installed by `hop claude-hook install` now also registers a `Stop` hook that bumps a per-turn counter at `<HOP_HOME>/claude-sessions/<HOP_SESSION>.turn` (`{ sessionId, count, at }`) each time Claude finishes a turn. A driver (the hop MCP) can wait on the counter advancing instead of screen-scraping for "agent done" — exact completion, no false early/late firing. Re-run `hop claude-hook install` to add it to an existing SessionStart-only install; it's a no-op outside hop and never disrupts Claude.
