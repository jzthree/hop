...
// Who may register is the admin's call (`hop registration domain <d>`); there
// is deliberately no built-in default. Subdomains of the configured domain
// count (cs.example.edu under example.edu), because plenty of departments
// hand out mail there.
...
const RESERVED_SUBDOMAINS = new Set([
  ...
  // The zone's OWN live services and legacy pre-prefix users. Registrant
  // hostnames carry a hop- prefix so they cannot collide with these, but a
  // registrant must not be able to CLAIM one of these labels either.
  'kat', 'kathy', 'jianzhou', 'jian', 'ssh-nas', 'nas', 'qnap', 'myssh', 'ssh'
]);
 ...
 */
function validateEmail(raw, { allowedDomain = null } = {}) {
  if (!allowedDomain || !String(allowedDomain).trim()) {
    return { ok: false, error: 'Registration is not open: no email domain has been configured.' };
  }
  const value = String(raw || '').trim().toLowerCase();
...
class RegistrationStore {
  ...
  constructor(filePath, options = {}) {
    ...
    this.allowedDomain = options.allowedDomain || null;
   ...
   */
  submit({ subdomain, email, fullName = '', department = '', areaOfExpertise = '', ip = '', skipEmailVerification = false }) {
    ...
    // When identity is proven by an external gate (Cloudflare Access proves
    // the @uchicago.edu login itself), there is no email to confirm: the
    // request skips pending_email and lands straight in the admin's queue.
    // Access will re-prove the same email at CLAIM time, so a fabricated
    // address here simply never claims. In that mode no verify token exists.
    const token = skipEmailVerification ? null : newToken();
    const base = {
      subdomain: nameCheck.value,
      email: emailCheck.value,
      fullName: String(fullName || '').slice(0, 120),
      department: String(department || '').slice(0, 120),
      areaOfExpertise: String(areaOfExpertise || '').slice(0, 300),
      createdAt: previous?.createdAt || now(),
      updatedAt: now(),
      requestIp: String(ip || '').slice(0, 64)
    };
    all[nameCheck.value] = skipEmailVerification
      ? { ...base, status: 'pending_approval', verifiedAt: now(), verifiedVia: 'cloudflare-access' }
      : {
          ...base,
    ...
    this.save(all);
    return { ok: true, subdomain: nameCheck.value, email: emailCheck.value, token, status: all[nameCheck.value].status };
  }
  ...
  /**
   * Cloudflare-Access claim: no token — the caller has already PROVEN this
   * email via the Access JWT, so the approved registration for that exact
   * email is the one to hand over. Case-insensitive on the address; the
   * gate normalises, but a claimant typing their own address never does.
   */
  resolveClaimByEmail(email) {
    const addr = String(email || '').trim().toLowerCase();
    if (!addr) return { ok: false, error: 'No authenticated email.' };
    const entry = Object.values(this.load()).find(
      (r) => String(r.email || '').toLowerCase() === addr && r.status === 'approved'
    );
    if (!entry) return { ok: false, error: 'No approved registration for your account yet.' };
    return { ok: true, registration: entry };
  }
